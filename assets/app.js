...
(function () {
  ...
  function bootIntroMotion() {
    ...
    //
    // 2026-08-12 열아홉 번째 라운드 — 사용자 요청: "ANZINE 문구가 완성되면 바로
    // 메인 콘텐츠로 떨어지지 말고 스크롤 한두번 하면 떨어지게." .intro의 높이를
    // 160svh→240svh로 늘리고(css.mjs) CONVERGE_END를 .75→.45로 낮췄다 — 합쳐지는
    // 애니메이션 자체가 차지하는 절대 스크롤 거리(픽셀 기준)는 이전과 비슷하게
    // 유지하면서(상자가 커진 만큼 비율을 낮췄다), 완성 이후 홀드 구간(0.45~1, 즉
    // 여유분의 55%)이 뷰포트 높이의 약 77vh로 훨씬 길어졌다 — 완성 상태를 보고도
    // 한동안 계속 스크롤해야 본문이 나온다.
    var CONVERGE_START = 0.13;
    var CONVERGE_END = 0.45;
