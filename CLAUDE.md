...
>
> **2026-09-08 구조 전환:** 보드를 **Claude Artifact → 로컬 웹앱**(`app/`)으로 바꿨다. 단일파일·CDN·`window.claude` 제약을 걷어내 편집성을 확보하고, 이유 생성은 **구독제 `claude -p`**(헤드리스)로 돌린다(API 키·과금 안 씀). 공유는 **설정 → HTML로 내보내기**(서버 없이 열리는 한 파일, `GET /export/<id>.html`)로 한다. 옛 단일 HTML 은 지웠다(git 이력의 `app/public/index.html` 초판이 그것).
...
└── data/runs/<id>.json          # 보드 기록. 업로드 한 번 = 파일 하나(차트 + 해석 전부). 보드 "기록" 탭이 읽는다
```
...
두 경로가 있다.
- **기본 차트**: 코드 안의 `DEMO_MATRIX`(순위 매트릭스, 해석 없음). 기록(`data/runs`)이 하나도 없을 때만 보인다. 미리 써 둔 이유·묶음 시드는 없앴다.
...
- **해석 단위는 항목이 아니라 그룹이다(2026-09-08).** "분석하기" = 상위 그룹(음악/영화/OTT/…) 9개마다 `buildGroupReadPrompt` 1콜 → `headline`/`popHead`/`summary`/`points`/`keywords`/`anomalies`(특이점 항목 2~3개, itemId 지목). 이미 있는 그룹은 다시 부르지 않는다. 동시성은 서버가 게이트한다(`TREND_CONCURRENCY`, 기본 3) — 클라이언트 타임아웃은 **대기열까지 포함**해 잡는다(그룹당 ~2분 × 3라운드 → 600초).
- 순위 변동·같은 팀 복수 진입·다른 차트 교차 등장은 **코드가 계산해**(`computeFacts`) 사실 태그로 프롬프트에 붙인다. LLM 은 사실을 받고 해석만 쓴다. 네 프롬프트가 공유하는 해석 원칙은 `prompts.mjs` 의 `LENS` 하나.
- 그룹 해석 3개 이상 → 주간 한 줄(`makeTopline`) 1콜 → 연관 신호 Auto(`runAuto`) 1콜이 자동으로 이어진다. 총 11콜.
- **항목별 LLM 해석은 없다.** 칸 팝업은 사실 태그 + (그룹 해석이 지목했으면) 특이점 한 줄. 모든 칸이 같은 모양이다. 그룹 해석 헤드라인은 **표 바로 위 가로 마퀴 한 줄**(`readsMarqueeEl`, 9개가 왼쪽으로 흐름, 그룹당 7초, 호버 시 정지)에 있고, 표 헤더에는 그룹명 + 빨간 네모(해석 있음)만 남는다. 둘 다 호버하면 특이점 칸이 밝아지고, 클릭하면 가운데 모달로 전문이 열린다. 카드 줄·호버 팝업·표 위 목록은 없다. 그 아래 전광판은 한 카테고리씩 다섯 칸을 2.6초마다 굴린다.
...
### 4.4 부가 기능
- **묶음 추론(bundles)**: 카테고리를 가로지르는 패턴을 묶어 한 편의 글로 만든다. 화면에선 **손패 카드 5장**(앞면 제목만) — 누르면 뒤집히고 모달로 본문·항목·키워드·좋아요가 뜬다. 아래 선택 바(직접 묶기)와는 별개.
...
- **내보내기**: 화면에 있는 그대로 CSV로 뽑는다(`buildChartCsv` → 브라우저 Blob 다운로드). **HTML로 내보내기**는 지금 기록(차트 + 해석 + 묶음)을 서버 없이 열리는 한 파일로 만든다 — `server.mjs` `buildExport` 가 CSS 인라인 + ES 모듈 6개를 data: URL importmap + 기록 JSON 을 `window.__RUN__` 으로 품는다. 그 파일에서는 AI·기록 동기화가 꺼진다(`EXPORTED`). React·XLSX·서체는 CDN 이라 인터넷은 필요.
...
- 결과물: `data/weekly/week{n}.xlsx` — §4.1 엑셀 형식 그대로. **자동 열은 새로 긁고, 수동 열은 `app/public/js/data.mjs` 의 `DEMO_MATRIX` 를 그대로 옮겨 채운다**(보드 업로드는 매트릭스 전체를 갈아끼우므로, 한 파일에 전 카테고리가 다 있어야 수동 카테고리가 사라지지 않는다).
...
- **디자인은 스위스 타이포그래피**(2026-09-08): 흰 종이·검정 잉크·그로테스크 한 가족(Pretendard, 라틴 폴백 Helvetica Neue/Arial)·모서리 0·그림자 0. 빨강(`--accent`, 스위스 레드)은 신호에만 쓴다 — 상승 배지, 활성 탭 밑줄, 특이점 칸 테두리, 이번 주 한 줄의 윗괘선. 라벨·칩·숫자는 검정(`--accent-ink` 는 검정에 매핑돼 있다). 세리프·모노·대문자 트래킹 라벨·알약 칩을 다시 넣지 않는다.
- **좁은 화면(≤720px)** 은 18열 표 대신 카테고리 탭 + 세로 한 열(`mobileChartEl`). 칸·팝업·특이점 강조는 데스크톱과 같은 `Cell` 을 쓴다.
...
- 켜면 이번 주 기본 차트가 이미 채워져 있다. xlsx 를 업로드하고 "카테고리 해석 만들기"를 누르면 그룹 해석 9개 → 이번 주 한 줄 → 연관 신호 묶음 5개가 자동으로 이어진다(실측 약 15분: 그룹 6.5분 + 한 줄 2.5분 + 묶음 5분). 결과는 `data/runs/` 에 남고 "기록" 탭에서 다시 연다.
