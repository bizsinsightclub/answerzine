...
function statCard(stat, story) {
  ...
  const spark = stat.trend?.length >= 2
    ? raw(sparklineSVG(stat.trend, color, { label: stat.label ?? "추이" }))
    : "";
  return h`<aside class="rail" style="${raw(dcVar(story))}">
  <div class="stat-card">
    <div class="label">${stat.label}</div>
    <div class="stat-value">${stat.value}</div>
    <div class="stat-spark" style="color:var(--dc,currentColor)">${spark}</div>
...
function pullquote(quote, story) {
  ...
  return h`<figure class="pullquote" style="${raw(dcVar(story))}">
  ${quote.text}
  ${ins
